...
# ============================================================
#  Comprimir PDF - reduz o tamanho de PDFs (Ghostscript)
#  Desenvolvido por Pablo Murad - 2026
# ============================================================

try { [Console]::OutputEncoding = [System.Text.Encoding]::UTF8 } catch {}
try { $Host.UI.RawUI.WindowTitle = 'Comprimir PDF' } catch {}
...
$configDir  = Join-Path $env:APPDATA 'ComprimirPdf'
...
Abrir 'Comprimir PDF'
# O Ghostscript costuma NAO estar no PATH: procura tambem onde ele se instala.
$gsExe = $null
foreach ($n in @('gswin64c','gswin32c')) {
    $c = Get-Command $n -ErrorAction SilentlyContinue
    if ($c) { $gsExe = $c.Source; break }
}
if (-not $gsExe) {
    foreach ($base in @("$env:ProgramFiles\gs", "${env:ProgramFiles(x86)}\gs")) {
        if (-not (Test-Path -LiteralPath $base)) { continue }
        $achado = Get-ChildItem -LiteralPath $base -Recurse -Filter 'gswin*c.exe' -ErrorAction SilentlyContinue |
                  Sort-Object FullName -Descending | Select-Object -First 1
        if ($achado) { $gsExe = $achado.FullName; break }
    }
}
if (-not $gsExe) {
    Write-Host ''
    Write-Host '[ERRO] Ghostscript não foi encontrado (é ele que comprime o PDF).' -ForegroundColor Red
    Write-Host 'Instale com:  winget install ArtifexSoftware.GhostScript' -ForegroundColor White
    Pausar; exit 1
}
Write-Host ("[OK] Ghostscript: {0}" -f $gsExe) -ForegroundColor Green
...
$ent = Pedir-Entrada $padrao @('.pdf') 'Pasta ou PDF'
$itens = Listar $ent @('.pdf')
if ($itens.Count -eq 0) { Write-Host 'Nenhum PDF encontrado.' -ForegroundColor Yellow; Pausar; exit 0 }

$niveis = @(
    [pscustomobject]@{ Id=1; Nome='Tela    - menor arquivo (72 dpi)';     V='/screen' },
    [pscustomobject]@{ Id=2; Nome='E-book  - equilibrado (150 dpi)';      V='/ebook' },
    [pscustomobject]@{ Id=3; Nome='Impressão - boa qualidade (300 dpi)';  V='/printer' },
    [pscustomobject]@{ Id=4; Nome='Gráfica - máxima qualidade';           V='/prepress' }
)
$idPadrao = if ($config -and $config.UltimoNivel) { [int]$config.UltimoNivel } else { 2 }
Titulo 'Nível de compressão'
foreach ($n in $niveis) {
    $m = if ($n.Id -eq $idPadrao) { '>' } else { ' ' }
    $cor = if ($n.Id -eq $idPadrao) { 'White' } else { 'Gray' }
    Write-Host ("{0} [{1}] {2}" -f $m, $n.Id, $n.Nome) -ForegroundColor $cor
}
Write-Host "Enter = usar a opção padrão [$idPadrao]" -ForegroundColor DarkGray
$nivel = $null
while (-not $nivel) {
    $e = Read-Host 'Nível (1-4)'; if ([string]::IsNullOrWhiteSpace($e)) { $e = "$idPadrao" }
    $nivel = $niveis | Where-Object { $_.Id -eq ($e -as [int]) } | Select-Object -First 1
    if (-not $nivel) { Write-Host '[!] Escolha de 1 a 4.' -ForegroundColor Yellow }
}

$destino = Pedir-Saida $(if ($config -and $config.UltimaSaida) { $config.UltimaSaida } else { Join-Path $ent.Origem 'Comprimido' })
Salvar-Config ([pscustomobject]@{ UltimaEntrada=$ent.Origem; UltimoNivel=$nivel.Id; UltimaSaida=$destino })
...
foreach ($f in $itens) {
    ...
    $antes[$saida] = $f.Length
    $a = @('-sDEVICE=pdfwrite','-dCompatibilityLevel=1.4',"-dPDFSETTINGS=$($nivel.V)",
           '-dNOPAUSE','-dQUIET','-dBATCH','-dSAFER',"-sOutputFile=$saida",$f.FullName)
    $tarefas += @{ Exe=$gsExe; Rotulo=$f.Name; Alvo=$saida; Args=$a }
}
...
if ($tarefas.Count -gt 0) {
    ...
    Write-Host ("{0} PDF(s), até {1} ao mesmo tempo..." -f $tarefas.Count, $lim) -ForegroundColor Gray
    $rs = Executar-EmParalelo -Tarefas $tarefas -Limite $lim
    Relatar $rs $tarefas ([ref]$ok) ([ref]$err)
    Write-Host ''
    $totA = 0; $totD = 0
    foreach ($t in $tarefas) {
        if (Test-Path -LiteralPath $t.Alvo) {
            $a = $antes[$t.Alvo]; $d = (Get-Item -LiteralPath $t.Alvo).Length
            $totA += $a; $totD += $d
            $pc = if ($a -gt 0) { [int]((1 - ($d / $a)) * 100) } else { 0 }
            Write-Host ("  {0,-42} {1,10} -> {2,10}   ({3}% menor)" -f $t.Rotulo, (Tam $a), (Tam $d), $pc) -ForegroundColor Gray
        }
    }
    if ($totA -gt 0) {
        Write-Host ''
        Write-Host ("TOTAL: {0} -> {1}   ({2}% menor)" -f (Tam $totA), (Tam $totD), [int]((1 - ($totD/$totA)) * 100)) -ForegroundColor Cyan
    }
}
